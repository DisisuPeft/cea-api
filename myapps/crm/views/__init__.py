from .leads import LeadsView, LeadView, CreateLeadFromLanding, RequestView, EstadisticsLeadsView, GetVendedoresView
from .pipeline import PipelineAllView, GetProgramsView, GetUnidadAcademicaView, GetEmpresaView, PipelineUpdateView, GetEtapasPipelineView
from .genericos import GetEstatusView, GetFuentesView, UpdateFuentesView, UpdateEstatusView
from .campanias import CampaniaViewSet, CampaniaProgramaGenericView
from .unidades_negocio import UnidadNegocioView